...
public class UserInterfaceImpl implements IUserInterfaceContract.View, EventHandler<KeyEvent>{
	...
	//how do we ekeep track of 81 different textfield
	private HashMap<Coordinates, SudokuTextField> textFieldCoordinates;
	
	private IUserInterfaceContract.EventListerner listener;
	...
	private void drawGridLines(Group root) {
		...
		while(index < 8) {
			...
			Rectangle horizontalLine = getLine(BOARD_PADDING,
					xAndY + 64 * index,
					thickness,
					BOARD_X_AND_Y);
	...
	private void drawBackgorund(Group root) {
	...
	@Override
	public void setListener(EventListerner listener) {
	...
	public void handle(KeyEvent e) {
		if(e.getEventType() == KeyEvent.KEY_PRESSED) {
			if(e.getText().matches("[0-9]")) {
				...
				handleInput(value, e.getSource());
			} else if (e.getCode() == KeyCode.BACK_SPACE) {
				handleInput(0, e.getSource());
			} else {
